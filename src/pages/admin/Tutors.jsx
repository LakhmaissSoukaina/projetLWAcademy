...
export default function Tutors() {
  ...
  const stats = {
    ...
    avgScore: tutors.length > 0 
      ? (tutors.reduce((acc, t) => acc + (t.score || 0), 0) / tutors.length).toFixed(1)
      : 0
  };
  ...
  const toggleTutorStatus = async (id) => {
    ...
    try {
      ...
      await fetchTutors(); // Rafraîchir la liste
    } catch (error) {
  ...
  const filteredTutors = tutors.filter(tutor => 
    ...
    tutor.studentId.includes(searchTerm)
  );
    ...
    <div className="space-y-6 w-full max-w-full">
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          <div>
            <span className="text-blue-900 font-semibold uppercase tracking-widest text-xs">Academic Excellence</span>
            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mt-2 font-serif">Peer Tutor Selection</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-all">
          ...
          <div className="w-full h-1 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-blue-900 rounded-full" style={{ width: `${(stats.potential / (stats.potential + stats.current)) * 100 || 0}%` }} />
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-all">
          ...
          <div className="w-full h-1 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-gray-500 rounded-full" style={{ width: `${(stats.current / (stats.potential + stats.current)) * 100 || 0}%` }} />
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
              ...
              {filteredTutors.map((student) => (
                <tr key={student.id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-6 py-6">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full font-bold text-xs ${student.rankColor || 'bg-gray-100 text-gray-500'}`}>
                      {student.rank}
                    </div>
                   </td>
                  <td className="px-6 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{student.program} • ID: {student.studentId}</p>
                      </div>
                    </div>
                   </td>
                  <td className="px-6 py-6">
                  ...
                  <td className="px-6 py-6">
                    <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase ${student.specializationColor}`}>
        ...
        <div className="p-6 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center">
          <p className="text-xs text-gray-400">
            Showing top <span className="font-bold text-gray-900">{filteredTutors.length}</span> performance leaders
          </p>
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-blue-100">
                Designated Peer Tutors will be granted 'Academic Assistant' privileges in their respective chapters.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                A minimum score of 95% in chapter-specific quizzes and 90% attendance is required for tutor eligibility.
              </p>
            </div>
          </div>
        </div>

...
export default function Dashboard() {
  ...
  // Écouter l'événement émis par la Navbar
  useEffect(() => {
  ...
  // Définition des sections avec leurs mots‑clés associés
  const sections = [
    { id: "kpi", title: "KPIs (Total Users, Pending Profs, Active Courses, Live Sessions)", keywords: ["total users", "utilisateurs", "pending profs", "professeurs", "active courses", "cours", "live sessions", "sessions"] },
    { id: "userManagement", title: "User Management", keywords: ["user", "utilisateur", "management", "gestion", "user management"] },
    { id: "professorValidation", title: "Professor Validation", keywords: ["professor", "professeur", "validation", "pending", "en attente"] },
    { id: "pedagogicalHierarchy", title: "Pedagogical Hierarchy", keywords: ["pedagogical", "hierarchy", "hiérarchie", "curriculum"] },
    { id: "weeklyEngagement", title: "Weekly Engagement", keywords: ["weekly", "engagement", "hebdomadaire", "server load", "active streams"] },
    { id: "broadcast", title: "Broadcast Notification", keywords: ["broadcast", "notification", "alerte", "send"] },
    { id: "contentSupervision", title: "Content Supervision", keywords: ["content", "supervision", "contenu", "flags", "copyright"] },
    { id: "footer", title: "Footer Info", keywords: ["footer", "logged", "system"] }
  ];
  ...
  const hierarchy = [ /* ... inchangé ... */ ];
  const systemFlags = [ /* ... inchangé ... */ ];

  return (
    <div className="space-y-6 w-full max-w-full">
      
      {/* Welcome Header - toujours visible (mais peut être masqué si vous voulez) */}
      <div>
        ...
        <p className="text-gray-500 text-base lg:text-lg max-w-2xl">
          Manage your educational ecosystem with precision...
        </p>
        {searchTerm && (
          <div className="mt-3 inline-flex items-center gap-2 bg-blue-50 text-blue-800 px-3 py-1 rounded-full text-sm">
            <span className="material-symbols-outlined text-sm">search</span>
            Showing only sections matching: <strong>{searchTerm}</strong>
          </div>
        )}
      </div>

      {/* KPI Grid - conditionné selon mots-clés */}
      {isSectionVisible(sections.find(s => s.id === "kpi").keywords) && (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        {isSectionVisible(sections.find(s => s.id === "userManagement").keywords) && (
          <div className="xl:col-span-8 bg-white rounded-xl shadow-sm border border-gray-50 overflow-hidden">
                  ...
                  {users.slice(0, 5).map(u => {
                    const userItem = {
                      ...
                      role: (() => {
                        ...
                        if (u.roles?.includes('ROLE_PROF')) return "Professor";
                        if (u.roles?.includes('ROLE_TUTEUR')) return "Tutor";
                        if (u.roles?.includes('ROLE_ETUDIANT')) return "Student";
                        return "User";
                      })(),
                      activity: (() => {
                      ...
                      avatar: `${(u.prenom?.charAt(0) || '')}${(u.nom?.charAt(0) || '')}`,
                      avatarColor: (() => {
                        if (u.roles?.includes('ROLE_ADMIN')) return "bg-blue-900 text-white";
                        if (u.roles?.includes('ROLE_PROF')) return "bg-blue-100 text-blue-800";
                        if (u.roles?.includes('ROLE_TUTEUR')) return "bg-amber-100 text-amber-800";
            ...
            <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 bg-gray-50/30">
              <span className="text-xs text-gray-400">Showing 5 of {stats.totalUsers} users</span>
        ...
        {isSectionVisible(sections.find(s => s.id === "professorValidation").keywords) && (
      ...
      {isSectionVisible(sections.find(s => s.id === "pedagogicalHierarchy").keywords) && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-7 space-y-6">
            {/* Weekly Engagement */}
            {isSectionVisible(sections.find(s => s.id === "weeklyEngagement").keywords) && (
            ...
            {isSectionVisible(sections.find(s => s.id === "broadcast").keywords) && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-50 p-5 lg:p-6">
      ...
      {/* Content Supervision */}
      {isSectionVisible(sections.find(s => s.id === "contentSupervision").keywords) && (
        <section>
      ...
      {/* Footer Info */}
      {isSectionVisible(sections.find(s => s.id === "footer").keywords) && (
